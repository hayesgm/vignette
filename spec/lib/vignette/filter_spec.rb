...
" }
      ...
      it "should raise error" do
        Vignette.with_settings(nil, session, nil) do
          ...
          expect(Vignette.tests).to eq({"vignette_(haml:3)" => "three"})
        end
      end
    end
...
" }
      ...
      it "should correctly call vignette from render" do
        Vignette.with_settings(nil, session, nil) do
          ...
          expect(html).to match(/\<p\>\s+three\s+\<\/p\>\n/)
          expect(Vignette.tests).to eq({"vignette_numbers" => "three"})
        end
      end
    end
    ...
    context "with a template file" do
      ...
      it "should correctly call vignette from render" do
        Vignette.with_settings(nil, session, nil) do
          ...
          expect(Vignette.tests).to eq({"vignette_(ex.html.haml:3)" => "scorpians"})
        end
      end
    end
  end
